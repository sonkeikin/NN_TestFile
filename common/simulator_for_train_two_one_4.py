...
class five_layer_simulator:
    def __init__(self, spacesize=310, currentTime_end=1000, f=3e+9, excitationPoint = 1 ,  w=80
    ...
    def run_simulator_test(self):
        ...
        t_test_five = []
        ##########################################################################
        fp_data_start = int((120-self.excitationPoint)+(120-self.Observation_point))
        fp_data_end = int(fp_data_start + self.w)
        fp_data_len = fp_data_end - fp_data_start
        print("需要截取范围" , fp_data_start , fp_data_end,"长度",fp_data_len)
        ##########################################################################
        print("-----------------------------テストデータを読み込む-----------------------------") 
                ...
                for layer_2_relativeEpsilon in self.test_two_layer_sample:
                    for layer_3_relativeEpsilon in self.test_three_layer_sample:
                        for c1 in range(self.spaceSize):
                            if 0 <= c1 <= 120:
                                self.relativeEpsilon[c1] = 1
                            elif 121 <= c1 <= 120 + 1 *self.layer_thick:
                                self.relativeEpsilon[c1] = layer_1_relativeEpsilon
                            elif (120 + 1*self.layer_thick + 1) <= c1 <= 120 + 2 *self.layer_thick:
                                self.relativeEpsilon[c1] = layer_2_relativeEpsilon
                            elif (120 + 2*self.layer_thick + 1) <= c1 <= self.spaceSize:
        ...
        return x,t_test_one,t_test_two,fp_data_len

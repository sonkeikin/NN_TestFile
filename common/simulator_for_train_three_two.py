...
class five_layer_simulator:
    def __init__(self, spacesize=320,layer_ait = 120, currentTime_end=1000, f=3e+9, excitationPoint = 1 ,  w=80
                 ,layer_thick=50 , Observation_point=2, Analytical_accuracy=1, con_layer = 3,
    ...
    def run_simulator_test(self):
        ...
        with tqdm(self.test_one_layer_sample) as t:
            for layer_1_relativeEpsilon in t:
                ...
                for layer_2_relativeEpsilon in self.test_two_layer_sample:
                    ...
                    for layer_3_relativeEpsilon in self.test_two_layer_sample:
                        #time_cost_layer3 = int(2 * self.layer_thick * ((layer_3_relativeEpsilon)**0.5))
                        step = 0
                        max_point = 0
                        while step <= 1:
                        ###生成关于第一层的观测波
                            for c1 in range(self.spaceSize):
                                if 0 <= c1 <= self.layer_air:
                                    ...
                                    self.relativeEpsilon[c1] = layer_1_relativeEpsilon
                                elif self.layer_air+1 + 1*self.layer_thick <= c1 <= self.layer_air + (2*self.layer_thick):
                                    self.relativeEpsilon[c1] = layer_2_relativeEpsilon
                                elif self.layer_air+1 + 2*self.layer_thick <= c1 <= self.spaceSize:
                            ...
                            ##########データを抽出し
                            Observation_point_E = Observation_point_E[(time_cost_air + time_cost_layer1 + time_cost_layer2) : (time_cost_air + time_cost_layer1 + time_cost_layer2 + self.w)]  
                        ...
                        ####前层特征值的规格化
                        xxxx = layer_2_relativeEpsilon / max(self.test_two_layer_sample)
        ...
        t_test_three = t_test_three.reshape(-1 , 1)
        return x,t_test_one,t_test_two,t_test_three
